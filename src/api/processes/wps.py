...
from ccfb.api.definitions.typing_definitions import AnyStr, Union, Optional, SettingDict
from ccfb.api.definitions.pyramid_definitions import HTTPInternalServerError, get_current_request, wsgiapp2, Registry
from ccfb.api.processes.types import PROCESS_WPS
...
def _get_settings_or_wps_config(
    ...
    wps_path = settings.get(ccfb_setting_name)
    if not wps_path:
...
def get_wps_path(settings):
    ...
    ccfb_url = get_base_url(settings)
    if wps_path.startswith(ccfb_url):
        return wps_path.replace(ccfb_url, '')
    return wps_path
