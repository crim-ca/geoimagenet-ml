#!/usr/bin/env python
# coding: utf-8
from ccfb.api.store.datatypes import Process, Job
from ccfb.api.rest_api import exceptions as ex, requests as r, schemas as s
from ccfb.api.store.factories import database_factory
from ccfb.api.store import exceptions as exc
from ccfb.api.definitions.pyramid_definitions import *
from ccfb.api.definitions.typing_definitions import AnyStr, Number, List, Union, Optional, UUID, TYPE_CHECKING
from ccfb.api.processes.types import process_categories, PROCESS_ML
from ccfb.api.processes.status import (
    map_status, STATUS_SUCCEEDED, STATUS_FAILED, STATUS_STARTED, STATUS_RUNNING
)
from ccfb.api.processes.utils import get_base_url
...
def process_ml_job_runner(self, job_uuid):
    ...
    # see:
    #   ``src.ml.impl.test_loader_from_configs`` for corresponding override
    worker_count = registry.settings.get('src.ml.data_loader_workers', 0)
